        ...
        <div className="container mx-auto p-4">
            <div className={`${cardBg} backdrop-blur rounded-lg shadow-lg p-6`}>
                <div className="flex justify-between items-center mb-6">
                    <h3 className={`text-xl font-semibold ${textOnDark}`}>登录位置</h3>
                </div>
                ...
                <form className="flex space-x-2 mb-6">
                    ...
                    {search && (
                        <Link to="/login-locations" className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">清除</Link>
                    )}
                </form>
                    ...
                    <table className="w-full whitespace-nowrap">
                        <thead>
                            <tr className="text-left font-bold">
                                ...
                                <th className="px-4 py-2">世界</th>
                                <th className="px-4 py-2">坐标</th>
                                <th className="px-4 py-2">IP地址</th>
                                <th className="px-4 py-2">登录时间</th>
                            </tr>
                        </thead>
                        <tbody>
                            {locations.data.length === 0 ? (
                                <tr><td colSpan="5" className="border px-4 py-2 text-center">没有找到记录</td></tr>
                            ) : locations.data.map((location) => (
                                <tr key={location.id}>
                                    <td className="border px-4 py-2">
                                        <div className="flex items-center space-x-2">
                                            <img src={`https://crafthead.net/avatar/${location.user.username}`} alt={location.user.username} className="w-6 h-6 rounded-sm" />
                                            <span>{location.user.username}</span>
                                        </div>
                                    </td>
                                    <td className="border px-4 py-2">{location.world}</td>
                                    <td className="border px-4 py-2">{isAdmin ? location.formatted_coordinates : '(*, *, *)'}</td>
                                    <td className="border px-4 py-2">{isAdmin ? location.ip : '***'}</td>
                                    <td className="border px-4 py-2">{location.login_at}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
