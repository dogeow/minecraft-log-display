        ...
        <div className="container mx-auto p-4">
            <div className={`${cardBg} backdrop-blur rounded-lg shadow-lg p-6`}>
                <div className="flex justify-between items-center mb-6">
                    <h3 className={`text-xl font-semibold ${textOnDark}`}>登录记录</h3>
                </div>
                ...
                <form className="flex space-x-2 mb-6">
                    ...
                    {search && (
                        <Link to="/logins" className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">清除</Link>
                    )}
                </form>
                    ...
                    <table className="w-full whitespace-nowrap">
                        ...
                        <tbody>
                            {logins.data.length === 0 ? (
                                <tr><td colSpan="4" className="border px-4 py-2 text-center">没有找到记录</td></tr>
                            ) : logins.data.map((login) => (
                                <tr key={login.id}>
                                    <td className="border px-4 py-2">
                                        <div className="flex items-center space-x-2">
                                            <img src={`https://crafthead.net/avatar/${login.user.username}`} alt={login.user.username} className="w-6 h-6 rounded-sm" />
                                    ...
                                    <td className="border px-4 py-2">{login.duration}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
